...
neuralNetwork::neuralNetwork(i64 psize_x, i64 psize_y, i64 pchannel, i64 pparallel, const string &i_filename,
                             ...
                             int headnum, int headdim, int attn_dim, int linear_dim) : pic_size_x(psize_x), pic_size_y(psize_y), pic_channel(pchannel), pic_parallel(pparallel),
...
{
    i64 block_len = len * channel_out;
    // decompose scale to c*2^m so that it can be calculated by bit shifting and integer mult
    int c, m;
    pair<int, int> pm = search(scale);
    m = pm.first;
    c = pm.second;
    // virtual_scale is the replaced scale
    ...
    for (i64 g = 0; g < block_len; ++g)
    {
        ...
        // val[0][s] is the verification witness
        val[0][s] = Fr(p * c * (1ll << (m + M + 1)) + (1 << M) - q * (1 << (M + 1))) * Fr(q * (1 << (M + 1)) + (1 << M) - c * (1ll << (m + M + 1)) * p);
        // it has to be negative, or quantization is wrong. This is just to make sure quantization is correct
    ...
    for (i64 g = 0; g < block_len; ++g)
    {
        ...
        /**
         * if the rounding is correct, we must have: p * c * 2^m - 0.5 ≤ q ≤ p * c * 2^m + 0.5
         * which means: p*c*2^(m+M+1) - 2^M ≤ q*2^(M+1) ≤ p*c*2^(m+M+1) + 2^M, where we multiplied 2^(M+1) to all sides
         * the circuit creates the following constraint: -c²·2^(2m+2M+2)·p² + c·2^(m+2M+3)·p·q - 2^(2M+2)·q² + 2^(2M) - s = 0
         * this is only satisfied if the rounding is correct.
         */
    ...
    calcNormalLayer(circuit, layer_id);
    // assert statements here are only for catching bugs, not providing cryptographic assurance to verifier
    // verifier still needs to verify them when running GKR
