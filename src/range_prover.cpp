...
/**
 * This function create a random number generator that generates many random values for testbench and testing the lookup functionality and speed.
 * In the real LLM proof settings, this function will not be used.
 */
...
/**
 * @param: x is the input array containing values we want to prove. It is of size m
 * @param: range is the padded bit length of values in x
 */
void range_prover::range_prove(ll *x, int range, int m, int thread_num)
{

    // fixed chunk size of 9 bits
    int log = 9; // range = 16 or 32
    assert(range % log == 0);
    // l represents how many 9-bit chunks are needed to cover the entire range
    // l also specify how many chunks do we have to divide the input into
    int l = range / log;
    // two dimentional array data
    // data[i][j] contains the i-th bit chunk of j-th input
    ll **data = new ll *[l];
    for (int i = 0; i < l; i++)
        data[i] = new ll[m];
    for (int i = 0; i < l; i++)
    ...
    // table t contains all valid values in 9-bit range
    ll *t = new ll[1 << log];
    for (int i = 0; i < (1 << log); i++)
    {
        t[i] = i;
    }
    // do the big lookup by calling logup multiple times
    // one logup verifies all input's single bit chunk
    for (int i = 0; i < l; i++)
    {
...
/**
 * This function handles one vector and one lookup table case
 * For the real lookup used in demo, multiple calls to this function is needed
 * @param: f: array containing all values you want to prove
 * @param: t: lookup table
 * @param: m: size of f
 * @param: n: size of t
 */
void range_prover::logup(ll *f, ll *t, int m, int n, int thread)
{
    ...
    // count occurrences of each table entry
    for (int i = 0; i < m; i++)
        // c[i] means how many times value i appears
        c[f[i]]++;
    ...
    // compute reciprocals
    invVec(G, F, m);  // G[i]=1/(r+f[i])
    invVec(H, Hp, n); // H[i]=1/(r+t[i])
    Fr s1 = 0, s2 = 0;
    for (int i = 0; i < m; i++)
        s1 += G[i];
    for (int i = 0; i < n; i++)
